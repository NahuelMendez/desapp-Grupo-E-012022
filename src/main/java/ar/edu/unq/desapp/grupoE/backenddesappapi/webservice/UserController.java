...
public class UserController {
    ...
    public ResponseEntity<String> login(@RequestBody AuthUserDTO authUserDTO) throws UserException {
        User user = authUserDTO.createUser();
        User userAuth = userService.login(user);
        return ResponseEntity.ok()
                .header(HttpHeaders.AUTHORIZATION, jwtProvider.createToken(userAuth))
                .body("ok");
    }
